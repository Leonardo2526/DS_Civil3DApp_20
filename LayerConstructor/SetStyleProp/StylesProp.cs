...
namespace SetStyleProp
{
    class StylesProp
    {
        ...
        public StylesProp(ObjectId obID, PropertyInfo pf, Type obt, Type rootType, StyleBase stb, ArrayList changedStylesList, 
            IMongoDatabase db)
        {
            ObId = obID;
            PropInf = pf;
        ...
        {
            if (CheckRoot(RootType.Name) == true)
                return;

            StyleLayer styleLayer = new StyleLayer(stylebase, PropInf, ObjectType, RootType, Database);
            string layerName = styleLayer.GetLayerName();

            /*
            if (layerName != "" && layerName != stylebase.Name)
                stylebase.Name = layerName;
            ...
            if (SplitedLayerName.Count > 1)
            {
                ...
                foreach (string field in SplitedLayerName)
                {
                    ...
                    if (i == 1)
                    {
                        ...
                        string description = mongo.GetStyleDescription(field);
                        if (description != "")
            ...
            try
            {
                using (Transaction acTrans = Main.docCurDb.TransactionManager.StartTransaction())
                {
                    ...
                    */
                    ...
                    labelStyle.Properties.Label.Layer.Value = layerName;
        ...
        // Add the style name and parameters to the list of changed styles
        {
            ...
            StyleInfo styleinfo = new StyleInfo
            {
                ...
                typeName = RootType.Name.ToString()
            };
        ...
        bool CheckRoot(string rootName)
            //Except odd styles
        {
            bool exceptObject = false;

            List<string> exceptRootObjects = new List<string>()
            {
                "BandStylesRoot",
                "TableStylesRoot",
                "LabelSetStylesRoot"
            ...
            foreach (string obj in exceptRootObjects)
            {
                if (rootName == obj)
                    exceptObject = true;
            }
